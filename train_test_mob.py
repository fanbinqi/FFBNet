...
parser.add_argument('--lr', '--learning-rate',
                    default=0.0015, type=float, help='initial learning rate')
parser.add_argument('--momentum', default=0.5, type=float, help='momentum')

parser.add_argument('--resume_net', default='./weight/FSSD_mobile0304/FSSD_mobile_VOC_epoches_60.pth', help='resume net for retraining')
...
save_folder = os.path.join(args.save_folder, args.version + '0304')
...
log_file_path = save_folder + '/train' + '.log'# + time.strftime('_%Y-%m-%d-%H-%M', time.localtime(time.time())) + '.log'
if args.dataset == 'VOC':
...
if args.version == 'FSSD_vgg':
    from models.FSSD_vgg_fbq import build_net

    cfg = COCO_mobile_300
elif args.version == 'FSSD_mobile':
    from models.FSSD_Mob_FPN import build_net
    cfg = VOC_300
else:
...
if args.dataset == 'VOC':
    ...
    testset = COCODetection(
        COCOroot, [('2014', 'valminusminival')], None)
        ...
        if iteration % epoch_size == 0:
            ...
            if epoch % args.save_frequency == 0 and epoch > 0:
                torch.save(net.state_dict(), os.path.join(save_folder, args.version + '_' + args.dataset + '_epoches_' +
                                                          repr(epoch) + '.pth'))
            if epoch % args.test_frequency == 0 and epoch >= 10:
                net.eval()
                top_k = 200
